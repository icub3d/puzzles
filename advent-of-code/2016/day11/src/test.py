...
while frontier:
    ...
    for move in moves:
        for direction in directions:
            ...
            new_floors[floor + direction] = tuple(
                sorted(floors[floor + direction] + move)
            )

            if not correct(new_floors[floor]) or not correct(
                new_floors[floor + direction]
            ):
                continue
            ...
            if next not in cost_so_far or new_cost < cost_so_far[next]:
                ...
                priority = (
                    ...
                    # + sum(
                    #     [
                    #         len(floor) * (len(new_floors) - i - 1)
                    #         for (i, floor) in enumerate(floors)
                    #     ]
                    # )
                )  # silly manually tweakable heuristic factor
